...
def _do_re_fire():
    if _misfire_jobs and not _misfire_jobs.empty():
        ...
        if re_fire_job:
            try:
                category_code = _get_category_code(re_fire_job.country, re_fire_job.category)
                if re_fire_job.os == OSType.Android:
                    ...
                    run_ios_top_task(country=re_fire_job.country, query_date=re_fire_job.query_date,
                                     sub_category_code=category_code, feed=re_fire_job.feed_type)
                get_misfire_logger().info('done re-firing job: ' + str(re_fire_job))
            except Exception as e:
                get_misfire_logger().info('error re-firing job: ' + str(re_fire_job))
...
def _get_category_code(country, category_name):
    category_names = ios_game_category_names if country == OSType.IOS else android_game_category_names
